...
static void handle_server_disconnected(proxy_t *proxy) {
...
tpx_err_t proxy_handle_read(proxy_t *proxy, int is_client) {
    ...
    if (queue_empty(in_bufq)) {
        ...
        case TPX_FAILURE:
            ...
            return TPX_FAILURE;
        case TPX_EMPTY:
            log_system_err(LL_ERROR, "Buffer queue corrupted: queue is "
                           "nonempty but no data is found", TPX_ERR_PLAIN);
            return TPX_FAILURE;
        case TPX_SUCCESS:
        default:
    ...
    if (is_client && nbytes <= 0) {
        int ssl_err = SSL_get_error(proxy->ssl, nbytes);
        ...
        if (proxy_handle_ssl_failure(proxy, nbytes) == TPX_CLOSED) {
            if (!(ssl_err == SSL_ERROR_SYSCALL &&
                  (saved_errno == ECONNRESET || saved_errno == ETIMEDOUT)))
                proxy_process_data(proxy, is_client);
            return TPX_CLOSED;
        }
    ...
    for (;;) {
        ...
        default:
            ...
            while (real_buflen > out_bufq->read_idx &&
                   ...
                   > 0) {
                out_bufq->read_idx += (size_t)nsent;
            }
...
tpx_err_t proxy_handle_ssl_failure(proxy_t *proxy, int retcode) {
    ...
    case SSL_ERROR_SSL:
        if (ERR_GET_REASON(ERR_peek_error())
            ...
            log_proxy(LL_DEBUG, proxy, "client_disconnect",
                      "Client closed without close_notify", NULL);
            return TPX_CLOSED;
        }
